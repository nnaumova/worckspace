	...
	public void rebuild() {
		if (children.size() >= MAX && !isExtendOn) {
			...
			for (int i = 0; i < Model.alphabet.length(); i++) {
				DictionaryElem nodeElem = new DictionaryTopic(this.toString()
						+ Model.alphabet.substring(i, i + 1));
				DefaultMutableTreeNode topic = new DefaultMutableTreeNode(
						nodeElem);
			...
			for (Object o : copyChildren) {
				...
				MainForm.getInstance().getModel()
						.insertPerson((Employee) child.getUserObject());
			}
