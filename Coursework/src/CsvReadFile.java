...
public class CsvReadFile {
	...
	public static void writeIntoFiles(int[][] finaly) {
		File testf = new File("C:/учеба/курсач/data/" + sizeVector + "/test/" + country +".txt");
		File trainf = new File("C:/учеба/курсач/data/" + sizeVector + "/train/" + country +".txt");
		File testClass = new File("C:/учеба/курсач/data/" + sizeVector + "/test Class/" + country +".txt");
		File trainClass = new File("C:/учеба/курсач/data/" + sizeVector + "/train Class/" + country +".txt");
			...
			try {
				...
				for (int i = 0; i < finaly.length; i++) {
					...
					if (i % 5 == 0) {
						...
						testClas.println(classString); //сша китай индия бразилия
					} else {
						for (int j = 0; j < finaly[i].length; j++) {
	...
	public static void main(String[] args) throws IOException {
		String readFile = "C:/учеба/курсач/data/" + country + "/";
		...
		/*File testf = new File("C:/учеба/курсач/data/corrbrazilia.txt");
		File trainf = new File("C:/учеба/курсач/data/number.txt");
		File checkf = new File("C:/учеба/курсач/data/diagrambrazilia.txt");
		try {
			// проверяем, что если файл не существует то создаем его
			if (!testf.exists()) {
				testf.createNewFile();
